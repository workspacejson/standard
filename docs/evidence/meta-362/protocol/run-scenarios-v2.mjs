...
const OUT = '<output-dir>/scenario-results';
...
const S1_TREATMENT = `diff --git a/src/routes/checkout.ts b/src/routes/checkout.ts
--- a/src/routes/checkout.ts
+++ b/src/routes/checkout.ts
@@ -8,7 +8,7 @@ export function createCheckout(request: CheckoutRequest) {
-  const idempotencyKey = \`\${request.orderId}:\${request.attemptNumber}\`;
...
const S1_PERTURBED = `diff --git a/src/routes/checkout.ts b/src/routes/checkout.ts
--- a/src/routes/checkout.ts
+++ b/src/routes/checkout.ts
@@ -8,7 +8,7 @@ export function createCheckout(request: CheckoutRequest) {
-  const idempotencyKey = \`\${request.orderId}:\${request.attemptNumber}\`;
...
const S2_TREATMENT = `diff --git a/src/services/workspace.ts b/src/services/workspace.ts
--- a/src/services/workspace.ts
+++ b/src/services/workspace.ts
@@ -XX,XX +XX,XX @@ export function findCoChangePartners(ws: NormalizedWorkspace, path: string): string[] {
-  return partners;
...
const S2_PERTURBED = `diff --git a/src/services/workspace.ts b/src/services/workspace.ts
--- a/src/services/workspace.ts
+++ b/src/services/workspace.ts
@@ -XX,XX +XX,XX @@ export function findCoChangePartners(ws: NormalizedWorkspace, path: string): string[] {
-  return partners;
...
const S3_TREATMENT = `diff --git a/src/instance.rs b/src/instance.rs
--- a/src/instance.rs
+++ b/src/instance.rs
@@ -XX,XX +XX,XX @@ pub fn remove(&self, package: &mut PackageJson) {
       Strategy::VersionsByName => {
...
const S3_PERTURBED = `diff --git a/src/instance.rs b/src/instance.rs
--- a/src/instance.rs
+++ b/src/instance.rs
@@ -XX,XX +XX,XX @@ pub fn remove(&self, package: &mut PackageJson) {
       Strategy::VersionsByName => {
...
const ARMS = [
  { id: 'baseline', getDiff: (s) => s.diff },
  { id: 'treatment', getDiff: (s) => s.treatment },
  { id: 'perturbation', getDiff: (s) => s.perturbed },
];
...
for (const scenario of SCENARIOS) {
  ...
  for (const arm of ARMS) {
    ...
    const diff = arm.getDiff(scenario);
    ...
    for (let run = 0; run < RUNS_PER_ARM; run++) {
      ...
      try {
        ...
        if (result.status === 'COMPLETED') {
          ...
          allResults[scenario.id][arm.id].push({ status: 'UNAVAILABLE', reason: result.reason });
        }
      } catch (err) {
        console.log(`      ERROR: ${err.message}`);
        allResults[scenario.id][arm.id].push({ status: 'ERROR', reason: err.message });
      }
    }
  }
}
